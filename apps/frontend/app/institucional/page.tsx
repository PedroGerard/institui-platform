...
export default function InstitutionalPage() {
    ...
    const canConfigureAssociation = hasPermission('ASSOCIATION_CONFIGURE');

    useEffect(() => {
        setEditForm({
            name: activeAssociation?.name || '',
            cnpj: activeAssociation?.cnpjFormatted || activeAssociation?.cnpj || '',
            foundationDate: toDateInput(activeAssociation?.foundationDate)
        });
    }, [activeAssociation]);
    ...
    async function updateAssociation(event: FormEvent<HTMLFormElement>) {
        event.preventDefault();

        if (!associationId || !activeAssociation) {
            setAssociationFeedback({
                type: 'error',
                message: 'Selecione uma associacao ativa para atualizar.'
            });
            return;
        }
        ...
        try {
            ...
            const association = await api.updateAssociation(associationId, {
                name: editForm.name,
                cnpj: editForm.cnpj,
                foundationDate: editForm.foundationDate
            });
            ...
            setAssociationFeedback({
                type: 'success',
                message: `${association.name} atualizada com registro na trilha de auditoria.`
            });
        } catch (err: unknown) {
            setAssociationFeedback({
                type: 'error',
                message: err instanceof Error ? err.message : 'Erro ao atualizar associacao.'
            });
        } finally {
    ...
    return (
        <InstitutionalLayout title="Institucional" activePath="/institucional">
            <div className="mx-auto max-w-4xl space-y-6">
                ...
                <section className="rounded-lg border border-slate-800 bg-slate-900 p-6">
                    <div className="mb-5">
                        <h3 className="text-lg font-bold text-slate-100">Atualizar entidade ativa</h3>
                        <p className="mt-1 text-sm text-slate-400">
                            Edicao auditavel dos dados juridicos basicos usados por atas, estatutos, documentos e prestacao de contas.
                        </p>
                    </div>

                    {!hasAssociation && (
                        <div className="mb-4 rounded-lg border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-200">
                            Selecione ou cadastre uma associacao antes de atualizar dados institucionais.
                        </div>
                    )}

                    {hasAssociation && !hasOperator && (
                        <div className="mb-4 rounded-lg border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-200">
                            Selecione um operador ativo para registrar a alteracao na auditoria.
                        </div>
                    )}

                    {hasAssociation && hasOperator && !loadingPermissions && !canConfigureAssociation && (
                        <div className="mb-4 rounded-lg border border-rose-500/30 bg-rose-500/10 p-4 text-sm text-rose-200">
                            O operador selecionado nao possui permissao para configurar a associacao.
                        </div>
                    )}

                    <form onSubmit={updateAssociation} className="grid gap-4 md:grid-cols-[1.3fr_0.8fr_0.6fr_auto] md:items-end">
                        <label className="block">
                            ...
                            <input
                                ...
                                className="w-full rounded-lg border border-slate-800 bg-slate-950 px-4 py-2.5 text-sm text-slate-100 outline-none focus:border-blue-500 disabled:cursor-not-allowed disabled:opacity-60"
                        ...
                        <label className="block">
                            ...
                            <input
                                required
                                value={editForm.cnpj}
                                onChange={(event) => setEditForm((current) => ({ ...current, cnpj: event.target.value }))}
                                disabled={!hasAssociation}
                                className="w-full rounded-lg border border-slate-800 bg-slate-950 px-4 py-2.5 text-sm text-slate-100 outline-none focus:border-blue-500 disabled:cursor-not-allowed disabled:opacity-60"
                        ...
                        <label className="block">
                            ...
                            <input
                                required
                                type="date"
                                value={editForm.foundationDate}
                                onChange={(event) => setEditForm((current) => ({ ...current, foundationDate: event.target.value }))}
                                disabled={!hasAssociation}
                                className="w-full rounded-lg border border-slate-800 bg-slate-950 px-4 py-2.5 text-sm text-slate-100 outline-none focus:border-blue-500 disabled:cursor-not-allowed disabled:opacity-60"
                            />
                        </label>
                        <button
                            type="submit"
                            disabled={updatingAssociation || !hasAssociation || !hasOperator || loadingPermissions || !canConfigureAssociation}
                        ...
                        >
                            ...
                            {updatingAssociation ? 'Atualizando...' : 'Atualizar'}
                        </button>
                    </form>
                </section>
