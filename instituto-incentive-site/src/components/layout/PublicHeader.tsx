...
import { ChevronDown, Mail, Menu, ShieldCheck, X } from "lucide-react";
...
const transparencyLinks = [
  ...
  },
  {
    ...
    description: "Valores, aplicação dos recursos e fontes de conferência.",
  },
  {
    href: "/certificacoes-reconhecimentos",
    label: "Certificações e Reconhecimentos",
    description: "Certidões, alvarás e evidências institucionais.",
  },
];
...
export function PublicHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[var(--brand-border)] bg-white/95 shadow-sm shadow-[rgba(0,63,68,0.04)] backdrop-blur">
      ...
      <nav className="mx-auto flex min-h-[4.75rem] max-w-7xl items-center justify-between gap-4 px-5 sm:px-8">
        ...
        <div className="flex items-center gap-2">
          <Link
            href="/contato"
            className="hidden items-center justify-center gap-2 rounded-lg bg-[var(--brand-teal)] px-4 py-2 text-sm font-bold text-white shadow-sm shadow-[rgba(0,104,113,0.20)] transition hover:bg-[var(--brand-teal-dark)] sm:inline-flex"
          >
            <Mail size={17} />
            Fale conosco
          </Link>
        ...
        <div id="public-mobile-menu" className="border-t border-[var(--brand-border)] bg-white px-5 py-4 shadow-lg shadow-[rgba(0,63,68,0.08)] lg:hidden">
          <div className="mx-auto grid max-w-7xl gap-2">
            ...
            <Link
              href="/contato"
              onClick={() => setIsOpen(false)}
              className="mt-2 inline-flex items-center justify-center gap-2 rounded-lg bg-[var(--brand-teal)] px-4 py-3 text-sm font-bold text-white transition hover:bg-[var(--brand-teal-dark)]"
            >
              <Mail size={17} />
              Fale conosco
            </Link>
